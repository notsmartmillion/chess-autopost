...
def check_narration(script: Dict[str, Any], facts: Dict[str, Any], rep: Report) -> None:
    ...
    cap_claim = re.compile(
        ...
        # Nothing contrastive in between, and no crossing a clause boundary.
        # "cannot wander — but capturing is different" AFFIRMS the capture;
        # so does "cannot wander off — it can only take the rook" (Kasparov's
        # Immortal, held wrongly for exactly this). A dash, semicolon or
        # colon starts a new clause whose polarity is its own; the negation
        # only owns the words in its clause.
